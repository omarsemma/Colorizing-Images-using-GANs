...
from .ops import pixelwise_accuracy, preprocess, postprocess, peak_signal_to_noise_ratio, similarity_index, write_loss_csv
...
class BaseModel:
    ...
    def train(self):
        ...
        for epoch in range(self.options.epochs):
            ...
            for input_rgb in generator:
                ...
                self.sess.run([self.gen_train, self.accuracy,self.accuracy_ssim,self.accuracy_psnr], feed_dict=feed_dic)
                self.sess.run([self.gen_train, self.accuracy,self.accuracy_ssim,self.accuracy_psnr], feed_dict=feed_dic)

                lossD, lossD_fake, lossD_real, lossG, lossG_l1, lossG_gan, acc,acc_ssim,acc_psnr, step = self.eval_outputs(feed_dic=feed_dic)
                values= [
                    ("epoch", epoch + 1),
                    ("iter", self.iteration),
                    ("step", step),
                    ...
                    ("G gan", lossG_gan),
                    ("accuracy", acc),
                    ("accuracy ssim", acc_ssim),
                    ("accuracy psnr", acc_psnr)]
                progbar.add(len(input_rgb), values= values)

                # log model at checkpoints
                if self.options.log and step % self.options.log_interval == 0:
                    first_line = True if (epoch == 1 and step==self.options.log_interval) else False
                    write_loss_csv(values = values,first_line=first_line)
                    # enregisterement dans un fichier numpy
    ...
    def validate(self):
        ...
        for input_rgb in val_generator:
            ...
            self.sess.run([self.dis_loss, self.gen_loss, self.accuracy,self.accuracy_ssim,self.accuracy_psnr], feed_dict=feed_dic)
    ...
    def build(self):
        ...
        # Si la valeur de dis_fake est proche de 1 => le discriminateur s'est trompe et donc le generateur a réussi sa tache.
        ...
        self.accuracy_psnr = peak_signal_to_noise_ratio(self.input_color, gen,self.options.color_space)
        self.accuracy_ssim = similarity_index(self.input_color, gen,self.options.color_space)
    ...
    def eval_outputs(self, feed_dic):
        ...
        acc_psnr = self.accuracy_psnr.eval(feed_dict=feed_dic)
        acc_ssim = self.accuracy_ssim.eval(feed_dict=feed_dic)
        step = self.sess.run(self.global_step)

        return lossD, lossD_fake, lossD_real, lossG, lossG_l1, lossG_gan, acc, acc_ssim , acc_psnr, step
