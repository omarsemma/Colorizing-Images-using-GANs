...
def peak_signal_to_noise_ratio(true, pred,colorspace):
    postprocess(true, colorspace, COLORSPACE_LAB)
    postprocess(pred, colorspace, COLORSPACE_LAB)
    max_value =1
    return tf.image.psnr(true,pred,max_value)[1]

def similarity_index(true,pred,colorspace):
    postprocess(true, colorspace, COLORSPACE_LAB)
    postprocess(pred, colorspace, COLORSPACE_LAB)
    max_value =1
    return tf.image.ssim(true,pred,max_value)[1]


def write_loss_csv(values,first_line, filename='data_train.csv'):
    row = [x[1] for x in values]
    with open(filename,'a',newline='') as f:
        writer = csv.writer(f)
        if first_line :
            columns = [x[0] for x in values]
